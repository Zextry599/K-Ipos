...
class ReportManageController extends Controller
{
    ...
    public function exportTransaction(Request $req)
    {
        ...
        if($check_access->kelola_laporan == 1){
            ...
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadview('report.export_report_transaction', compact('dates', 'tgl_awal', 'tgl_akhir', 'market'));
            return $pdf->stream();
        }else{
            return back();
        }
    ...
    public function exportWorker(Request $req, $id)
    {
        ...
        if($check_access->kelola_laporan == 1){
            ...
            $pdf = \Barryvdh\DomPDF\PDF::loadview('report.export_report_worker', compact('transaksi', 'pasok', 'tgl_awal', 'tgl_akhir', 'id', 'jml_act_pasok', 'jml_act_trans', 'market'));
            return $pdf->stream();
        }else{
            return back();
        }
